...
# Prefer absolute package imports for static analyzers; fallback to code/ on failure
try:
...
# ----------------------------
CSV_PATH = os.path.join('sporulation', 'sporeinfo.csv')
BASE_DIR = os.path.join('sporulation', 'data')
...
# Default path for fixed best HPs summary
...
def _load_sequences_list(data_dir: str, labels_df: pd.DataFrame, seq_len: int, verbose: bool = True):
    labels_map = {row['file']: row['ability_TRUE'] for _, row in labels_df.iterrows()}
    sequences = []
    ...
    for i, file_name in enumerate(file_list):
        ...
        label = labels_map.get(file_name)
        if label is None:
            continue
...
def make_datasets(seq_len: int, epoch_budget: int, val_epoch_budget: int):
    ...
    if _TRAIN_SEQ_CACHE is None:
        _TRAIN_SEQ_CACHE = _load_sequences_list(train_dir, labels_df, seq_len, verbose=True)
    if _VAL_SEQ_CACHE is None:
        _VAL_SEQ_CACHE = _load_sequences_list(val_dir, labels_df, seq_len, verbose=True)
...
def train_one_trial(
    ...
    if getattr(args, 'robust_epsilon_only', False):
        ...
        eval_script = os.path.join('sporulation', 'code', 'evaluation.py')
        eval_dir = getattr(args, 'eval_dir', os.path.join('sporulation', 'data', 'eval'))
        test_dir = getattr(args, 'test_dir', os.path.join('sporulation', 'data', 'test'))
        ...
        print(f"[robust] Eval done. mean_sauc={sauc:.4f} acc={acc:.4f}", flush=True)
        obj = 0.9 * sauc + 0.1 * acc

        # Persist as current-best checkpoint if improved (atomic replace)
        try:
            robust_dir = os.path.join('sporulation', 'robust_model', f"{args.study_name}")
        ...
        # Persist best-so-far model under sporulation/robust_model/<study_name>
        try:
            robust_dir = os.path.join('sporulation', 'robust_model', f"{args.study_name}")
            os.makedirs(robust_dir, exist_ok=True)
            best_model_path = os.path.join(robust_dir, 'best_model.pth')
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Optuna HPO for sporulation CNN')
    # HPO controls
    p.add_argument('--tune', action='store_true', help='Run Optuna tuning')
    p.add_argument('--study-name', type=str, default='sporulation_cnn')
    ...
    p.add_argument('--eval-dir', type=str, default=os.path.join('sporulation', 'data', 'eval'), help='Evaluation directory for sporulation evaluation.py')
    p.add_argument('--test-dir', type=str, default=os.path.join('sporulation', 'data', 'test'), help='Test FASTA root for sporulation evaluation.py')
...
def main():
    ...
    # Export simple CSV of trials
    try:
        df = study.trials_dataframe()
        outdir = os.path.join('spore_optuna', f"{args.study_name}_sporulation")
