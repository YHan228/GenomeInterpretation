"""
--------------------------------------------------------------------------------
CHANGELOG (2025-08-27): Major HPO Restructuring for Unified Model Selection
  - Changed from multi-objective (NSGA-II) to single-objective optimization (TPE/Bayesian)
  - Objective: Maximize Saliency AUC only (removed Val Accuracy as separate objective)
  - Architecture parameters made quasi-continuous with log scales where appropriate
  - Expanded dataset parameters: GC values (added 0.725, 0.75, 0.775, 0.8), Conservation (added 0.65, 0.75)
  - CRITICAL: Now finds ONE best overall model across ALL datasets (not per-dataset)
  - Models evaluated on multiple datasets, objective returns average Saliency AUC
  - Total combinations: 13 GC x 5 Conservation = 65 datasets
--------------------------------------------------------------------------------
"""
...
GC_HPARAMS = [0.5, 0.53, 0.55, 0.575, 0.6, 0.625, 0.65, 0.675, 0.7, 0.725, 0.75, 0.775, 0.8] # n=13
CONS_HPARAMS = [0.6, 0.65, 0.7, 0.75, 0.8] # n=5
# ---
...
CONFIG = {
    ...
    'hpo': {
        ...
        'sampler': 'tpe',          # Changed to TPE for single-objective Bayesian optimization
...
SEARCH_SPACE = {
    'arch': {
        # Quasi-continuous parameters for better search efficiency
        'k1': {'low': 7, 'high': 41, 'step': 2},  # odd kernels, range 7-41
        'k2': {'low': 3, 'high': 11, 'step': 2},   # odd kernels, range 3-11
        'k3': {'low': 3, 'high': 11, 'step': 2},   # odd kernels, range 3-11
        'c1': {'low': 16, 'high': 128, 'log': True},  # channels, log scale
        'c2': {'low': 32, 'high': 256, 'log': True},  # channels, log scale
        'c3': {'low': 64, 'high': 384, 'log': True},  # channels, log scale
        'pool_w': {'low': 1, 'high': 100, 'log': True},  # pooling width, log scale
        'act1': ["exp", "relu", "gelu", "softplus"],  # expanded activation choices
    ...
    'opt': {
        ...
        'train_batch_size': {'low': 256, 'high': 1024, 'step': 128},  # made continuous
        'grad_clip': {'low': 0.5, 'high': 10.0},  # made continuous
    },
    'regimes': {
        'random_smoothing': {
...
def load_or_generate_dataset(gc_pos: float, conservation: float):
    ...
    # Ensure cache directory exists (important for multi-worker scenarios)
    os.makedirs(DATASET_CACHE_DIR, exist_ok=True)
    ...
    # Fix: Remove .npz from cache_path before adding temp suffix
    cache_base = cache_path.replace('.npz', '')
    temp_path = f"{cache_base}.{temp_id}.tmp.npz"
    ...
    try:
        ...
        # Atomic rename - if it fails because another worker already created it, that's okay
        try:
            os.rename(temp_path, cache_path)
        except FileExistsError:
            # Another worker already created the cache file, that's fine
            if os.path.exists(temp_path):
                os.remove(temp_path)
    except Exception as e:
        print(f"Warning: Could not cache dataset for gc={gc_pos:.3f}, cons={conservation:.2f}: {e}")
        ...
        if os.path.exists(temp_path):
            try:
                ...
                pass
        # Don't re-raise - just return the dataset without caching
...
def train_standard(model, train_loader, val_loader, loss_fn, optimizer, dev, scaler, writer, scheduler, epochs: int = 10, early_stopping_patience: int = 15, early_stopping_min_delta: float = 1e-4, trial=None, grad_clip: float = 5.0, trial_step_offset: int = 0) -> None:
    ...
    for epoch in range(epochs):
        ...
        if trial is not None and optuna is not None:
            try:
                trial.report(float(avg_val_loss), step=trial_step_offset + epoch)
...
def train_hotflip(model, train_loader, val_loader, loss_fn, optimizer, dev, scaler, writer, scheduler,
                  ...
                  early_stopping_patience: int = 25, early_stopping_min_delta: float = 1e-4, gc_pos: float = 0.5, trial=None, grad_clip: float = 5.0, trial_step_offset: int = 0) -> None:
...
def run_study(args):
    ...
    # Single-objective optimization with TPE (Bayesian optimization)
    if args.sampler == "tpe" or args.sampler is None:
        sampler = TPESampler()
        directions = ["maximize"]  # scalar objective: maximize saliency_auc
    elif args.sampler == "nsga2":
        # Keep backward compatibility but single objective
    ...
    def objective(trial: 'optuna.trial.Trial'):
        ...
        if mode == 'robust':
            ...
            # Architecture parameters - now mostly continuous
            k1 = trial.suggest_int('k1', SEARCH_SPACE['arch']['k1']['low'], SEARCH_SPACE['arch']['k1']['high'], step=SEARCH_SPACE['arch']['k1']['step'])
            k2 = trial.suggest_int('k2', SEARCH_SPACE['arch']['k2']['low'], SEARCH_SPACE['arch']['k2']['high'], step=SEARCH_SPACE['arch']['k2']['step'])
            k3 = trial.suggest_int('k3', SEARCH_SPACE['arch']['k3']['low'], SEARCH_SPACE['arch']['k3']['high'], step=SEARCH_SPACE['arch']['k3']['step'])
            # Ensure odd kernels
            k1 = k1 if k1 % 2 == 1 else k1 + 1
            k2 = k2 if k2 % 2 == 1 else k2 + 1
            k3 = k3 if k3 % 2 == 1 else k3 + 1
            
            c1 = trial.suggest_int('c1', SEARCH_SPACE['arch']['c1']['low'], SEARCH_SPACE['arch']['c1']['high'], log=SEARCH_SPACE['arch']['c1']['log'])
            c2 = trial.suggest_int('c2', SEARCH_SPACE['arch']['c2']['low'], SEARCH_SPACE['arch']['c2']['high'], log=SEARCH_SPACE['arch']['c2']['log'])
            c3 = trial.suggest_int('c3', SEARCH_SPACE['arch']['c3']['low'], SEARCH_SPACE['arch']['c3']['high'], log=SEARCH_SPACE['arch']['c3']['log'])
            pool_w = trial.suggest_int('pool_w', SEARCH_SPACE['arch']['pool_w']['low'], SEARCH_SPACE['arch']['pool_w']['high'], log=SEARCH_SPACE['arch']['pool_w']['log'])
            ...
            train_batch_size = trial.suggest_int('train_batch_size', SEARCH_SPACE['opt']['train_batch_size']['low'], SEARCH_SPACE['opt']['train_batch_size']['high'], step=SEARCH_SPACE['opt']['train_batch_size']['step'])
            grad_clip = trial.suggest_float('grad_clip', SEARCH_SPACE['opt']['grad_clip']['low'], SEARCH_SPACE['opt']['grad_clip']['high'])
        ...
        # ---------------- Training/Eval Across Multiple Datasets ----------------
        ...
        artifacts_dir = os.path.join(args.artifacts_dir, f"study_{study_name_actual}", f"trial_{trial.number}") if args.save_trial_artifacts else None
        if artifacts_dir:
        ...
        for ds_idx, (gc_val, cons_val) in enumerate(eval_dataset_keys):
            ...
            for seed_idx in range(num_seeds):
                ...
                # Build model with the suggested or fixed parameters
                params_for_model = {
                    'k1': k1, 'k2': k2, 'k3': k3,
                    'c1': c1, 'c2': c2, 'c3': c3,
                    'pool_w': pool_w,
                    'drop_conv1': drop_conv1, 'drop_conv2': drop_conv2,
                    'drop_conv3': drop_conv3, 'drop_fc': drop_fc,
                    'act1': act1,
                }
                ...
                writer = make_writer(log_dir=os.path.join(tb_root, f"dataset_gc{gc_val:.3f}_cons{cons_val:.2f}_seed_{seed}"))
                ...
                if artifacts_dir and not dataset_failed:
                    ...
                    with open(params_path, 'w') as f:
                        json.dump({
                            ...
                            'gc_pos': gc_val, 'conservation': cons_val,
                        }, f)
    ...
    # Get best trials (single objective: highest Saliency AUC)
    best_trials = []
    for t in completed:
    ...
    best_trials_sorted = sorted(best_trials, key=lambda x: x['saliency_auc'], reverse=True)
    top10 = best_trials_sorted[:10]

    # Also filter for high accuracy models (optional constraint)
    high_acc_trials = [t for t in best_trials if t['val_acc'] >= 0.85]
    high_acc_sorted = sorted(high_acc_trials, key=lambda x: x['saliency_auc'], reverse=True)
    top10_high_acc = high_acc_sorted[:10]

    summary = {
        'study_name': study_name_actual,
        'storage': storage,
        'optimization': 'single_objective_unified',
        'num_datasets': len(all_datasets),
        'counts': {'completed': len(completed), 'pruned': len(pruned), 'failed': len(failed), 'total': len(study.trials)},
        'top10_by_saliency_auc': top10,
        'top10_high_acc': top10_high_acc,
    ...
    # Print concise table
    if top10:
        print("\nTop 10 trials (sorted by average Saliency AUC across datasets):")
        for r in top10:
            print(f"  Trial {r['number']}: acc={r['val_acc']:.3f}, auc={r['saliency_auc']:.3f}, n_datasets={r['num_datasets_evaluated']}, key params={{k1:{r['params'].get('k1')}, c1:{r['params'].get('c1')}, pool_w:{r['params'].get('pool_w')}, lr:{r['params'].get('lr'):.5f}}}")
    
    if top10_high_acc:
        print("\nTop 10 high-accuracy trials (Val Acc >= 0.85, sorted by Saliency AUC):")
        for r in top10_high_acc[:5]:  # Show only top 5 for brevity
            print(f"  Trial {r['number']}: acc={r['val_acc']:.3f}, auc={r['saliency_auc']:.3f}")
    
    if study.best_trial:
        print(f"\nBest trial: {study.best_trial.number} with Saliency AUC = {study.best_value:.4f}")
...
def get_experiment_combos(experiment_mode: str) -> List[dict]:
    ...
    print(f"Total experiment combinations: {len(combos)} (2 schedules × {len(GC_HPARAMS)} GC values × {len(CONS_HPARAMS)} conservation values)")
    return combos
...
def train_gaussian_smoothing(model, train_loader, val_loader, loss_fn, optimizer, dev, scaler, writer, scheduler,
                             sigma2: float, epochs: int = 10, early_stopping_patience: int = 10, early_stopping_min_delta: float = 1e-4, trial=None, grad_clip: float = 5.0, trial_step_offset: int = 0) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sampler", type=str, choices=["nsga2", "tpe"], default='tpe', help="Optuna sampler to use. Default: TPE (Bayesian optimization)")
    ...
    parser.add_argument(
        "--eval-mode",
        type=str,
        choices=["panel", "all", "progressive"],
        default="all",
        help="Per-trial dataset coverage: panel (diverse fixed set), all (all 65), progressive (growing random).",
    )
    parser.add_argument(
        "--pessimism-alpha",
        type=float,
        default=0.0,
        help="Robust objective: return mean_auc - alpha * std_auc.",
    )
