#!/usr/bin/env python3
"""Genome snippet analysis with Taxonomic Holdout (blocked split).

Implements phylogenetically-aware validation for k-mer and GC% analysis:
- Entire taxonomic groups (Order/Family) are assigned exclusively to train/test
- Prevents phylogenetic leakage and provides honest generalization estimates

Features:
- GC% distribution by class
...
def create_taxonomic_file_split(
...
) -> Tuple[Dict[str, int], List[str], Dict[str, str], Dict[str, str]]:
    """Create train/test file splits where entire taxonomic groups are held out.

    Returns:
        labels_map: filename -> class_idx
        classes: list of class names
        file_to_split: filename -> "train" | "test"
        file_to_taxon: filename -> taxon name
    """
    rng = np.random.default_rng(seed)

    # Normalize filenames
    metadata_df = metadata_df.copy()
    metadata_df["fasta_norm"] = metadata_df["Fasta file"].apply(
        lambda x: Path(str(x)).name.strip().lower() if pd.notna(x) else ""
    )

    # Build labels map
    def _map_label(val) -> Optional[str]:
        if pd.isna(val):
            return None
    ...
    metadata_df["_label"] = metadata_df[phenotype].apply(_map_label)
    valid_df = metadata_df[metadata_df["_label"].notna() & metadata_df["fasta_norm"].str.len().gt(0)].copy()

    if taxon_level not in valid_df.columns:
        raise ValueError(f"Taxon level '{taxon_level}' not found in metadata")

    valid_df = valid_df[valid_df[taxon_level].notna()]

    # Build classes
    classes = sorted(valid_df["_label"].unique().tolist())
    class_to_idx = {c: i for i, c in enumerate(classes)}

    # Group by taxon
    taxon_to_files: Dict[str, List[str]] = {}
    file_to_label: Dict[str, str] = {}
    file_to_taxon: Dict[str, str] = {}

    for _, row in valid_df.iterrows():
        fname = row["fasta_norm"]
        taxon = str(row[taxon_level])
        label = row["_label"]
        taxon_to_files.setdefault(taxon, []).append(fname)
        file_to_label[fname] = label
        file_to_taxon[fname] = taxon

    # Filter taxa with minimum samples
    taxon_to_files = {t: files for t, files in taxon_to_files.items()
                      if len(files) >= min_samples_per_group}

    # Calculate target sizes
    all_files = [f for files in taxon_to_files.values() for f in files]
    n_total = len(all_files)
    n_val = int(n_total * val_ratio)
    n_test = int(n_total * test_ratio)

    # Shuffle taxa and assign to splits
    taxa = list(taxon_to_files.keys())
    rng.shuffle(taxa)
    ...
    for taxon in taxa:
        files = taxon_to_files[taxon]
        if len(val_files) < n_val:
    ...
    # Combine train + val for training (val used as part of CV)
    train_files = train_files + val_files
    train_taxa = train_taxa + val_taxa

    print(f"[Taxonomic Holdout] Level={taxon_level}")
    print(f"  Train: {len(train_files)} files from {len(train_taxa)} {taxon_level}s")
    print(f"  Test:  {len(test_files)} files from {len(test_taxa)} {taxon_level}s")
    print(f"  Train taxa: {train_taxa[:5]}{'...' if len(train_taxa) > 5 else ''}")
    print(f"  Test taxa:  {test_taxa}")

    # Build outputs
    labels_map = {fname: class_to_idx[file_to_label[fname]] for fname in all_files}
    ...
    return labels_map, classes, file_to_split, file_to_taxon


def gather_dataset_files_by_split(
    directories: Iterable[Path],
    labels_map: Dict[str, int],
    file_to_split: Dict[str, str],
    target_split: str,
) -> Dict[int, List[Path]]:
    """Gather files matching a specific split (train/test)."""
    ...
    for directory in directories:
        ...
        for entry in sorted(directory.iterdir()):
            ...
            if not lower_name.endswith(_FASTA_SUFFIXES):
                continue
            # Check if this file belongs to target split
            if file_to_split.get(lower_name) != target_split:
                continue
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Sample 1 Mbp snippets with Taxonomic Holdout split.")
    ...
    ap.add_argument("--taxon_level", type=str, default="Order", choices=["Order", "Family", "Genus"],
    ...
    ap.add_argument("--train_snippets", type=int, default=400, help="Number of 1 Mbp snippets to sample from train.")
    ...
    ap.add_argument("--val_ratio", type=float, default=0.15, help="Fraction of samples for validation (added to train for CV).")
    ap.add_argument("--test_ratio", type=float, default=0.15, help="Fraction of samples for test.")
    ap.add_argument("--out_dir", type=str, default=None, help="Output directory (defaults to phenotype/plots/genome_stats_blocked/<slug>).")
...
def main() -> None:
    ...
    # Collect all FASTA directories
    train_dir = data_root / "train"
    val_dir = data_root / "validation"
    test_dir = data_root / "test"

    all_dirs = [d for d in (train_dir, val_dir, test_dir) if d.is_dir()]
    if not all_dirs:
        raise FileNotFoundError(f"No FASTA directories found under {data_root}")
    ...
    # Create taxonomic holdout split
    print(f"[genome-stats] Creating taxonomic holdout split at {args.taxon_level} level", flush=True)
    labels_map, classes, file_to_split, file_to_taxon = create_taxonomic_file_split(
        metadata_df,
        phenotype=phenotype,
    ...
    # Gather files using taxonomic split (search all directories, filter by split assignment)
    train_files = gather_dataset_files_by_split(all_dirs, labels_map, file_to_split, "train")
    ...
    # Gather test files using taxonomic split
    test_files = gather_dataset_files_by_split(all_dirs, labels_map, file_to_split, "test")
    if test_files:
        ...
        print(f"[genome-stats] Warning: no test files assigned; evaluation will be skipped.", flush=True)
        test_df = pd.DataFrame()
    ...
    out_dir = Path(args.out_dir) if args.out_dir else Path("phenotype/plots/genome_stats_blocked") / f"{slug}_by_{args.taxon_level}"
    ensure_dir(out_dir)
    ...
    summary = build_summary(
        ...
        parameters={
            ...
            "taxon_level": args.taxon_level,
            "data_dirs": [str(d) for d in all_dirs],
